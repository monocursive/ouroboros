...
//! any method means is [`crate::server`]'s business.
//!
//! ## Two honesty properties, both bounded
//!
//! **Frames are capped as they are read, not after.** [`read_frame`] never lets `buf` grow past
//! `cap`: a line that runs long is truncated in place and its bytes are drained to the next
//! newline, so a peer cannot grow this process's memory by withholding a `\n`. A naive
//! `read_line` would buffer the whole oversize line first and only then notice — which is the
//! memory exhaustion the cap exists to prevent. It matters more here than it does for the
//! desktop helper: this process also holds a JIT and a guest heap.
//!
//! **A non-JSON line is noise, not an error to answer.** The peer is the trusted
//! `Ouroboros.Wasm.Pool`, not a stranger; garbage on the pipe means the pipe is broken, so
//! [`classify`] reports it and [`crate::server`] counts it toward a budget rather than spewing
//! an error frame per line.

use serde_json::Value;
use tokio::io::{AsyncBufRead, AsyncBufReadExt};

/// The default frame cap: 8 MiB. One line is a JSON-RPC message, not a file, and a line that
...
/// Classifies one line's bytes. "One JSON object per line" is taken literally: a JSON array or
/// scalar is not a message this helper speaks, so it is noise rather than a thing to answer
/// with an error.
...
mod tests {
    ...
    /// The cap is on memory, not just on what is returned, and this is the test that says so.
    ///
    /// `buf` is emptied on an oversize line either way, so asserting on its *contents* cannot
    /// tell a bounded read from one that buffered a megabyte and then threw it away. Capacity
    /// can: a `Vec` that only ever had `cap` bytes pushed into it never reserves more than
    /// twice that, and one that was handed the whole line reserves the whole line. Replace
    /// `push_bounded` with a naive `extend_from_slice` and this is the assertion that fails.
    #[tokio::test]
    async fn an_oversize_line_never_grows_the_buffer_past_the_cap() {
        ...
        assert_eq!(
            read_frame(&mut reader, &mut buf, CAP).await.unwrap(),
            Frame::Oversize
        );
        assert!(
            buf.capacity() <= 2 * CAP,
            "a {} byte line grew the buffer to {} bytes against a {CAP} byte cap",
            data.len(),
            buf.capacity()
        );
    }
