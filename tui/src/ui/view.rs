...
fn new_session(frame: &mut Frame, area: Rect, app: &App, dialog: &NewSession) {
    ...
    if !dialog.request.machine.trim().is_empty() {
        footer.push(Line::from(Span::styled(
            "Connected checks reachability, not provider readiness. Install and sign in to the provider on that destination; no credential is ever copied between machines.",
            Style::default().fg(theme::warn()),
        )));
        footer.push(Line::from(Span::styled(
