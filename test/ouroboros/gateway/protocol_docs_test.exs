...
defmodule Ouroboros.Gateway.ProtocolDocsTest do
  ...
  # Found by this test and recorded rather than quietly fixed: D6's two rewind verbs are in
  # `@table`, have `invoke/2` clauses, and appear nowhere in `docs/TUI.md` — §2.4's catalog
  # included. Deleting a name from this list is what closing the gap looks like.
  @tui_catalog_gaps ~w(interactive.rewind interactive.rewind_points)
