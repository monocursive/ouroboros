defmodule Ouroboros.ApplicationRecoveryTest do
  ...
  test "the model admission boundary owns every session that consumes its leases", context do
    live = start_controlled_session(context.workspace, "admission")
  ...
  test "the effect ledger owns the execution subtree beneath its durable boundary", context do
    live = start_controlled_session(context.workspace, "ledger")
    execution_monitor = Process.monitor(live.execution)
    coordinator_monitor = Process.monitor(live.coordinator)
    ledger = Process.whereis(Ouroboros.Agent.EffectLedger)
    jido = Process.whereis(Ouroboros.Jido)
  ...
  setup context do
    ...
    on_exit(fn ->
      ...
      restore_env(:ouroboros, :native_data_dir, previous_native_data_dir)
      File.rm_rf(base)
      assert {:ok, _started} = Application.ensure_all_started(:ouroboros)
    end)
  ...
  test "a killed interactive registry preserves workspace exclusion while sessions recover",
    ...
    assert_receive {:ouroboros_test_model_started, _run_id, %RunRequest{}, adapter}, 1_000
    ...
    {:ok, before} = InteractiveSession.info(ref)
    {:ok, execution} = Ouroboros.Session.info(before.harness_session_id)
    ...
    refute_receive {:DOWN, ^execution_monitor, :process, _, _}, 100
    assert {:ok, still_live} = Ouroboros.Session.info(before.harness_session_id)
    assert still_live.pid == execution.pid
    assert still_live.generation == execution.generation
    assert still_live.active_turn_id == execution.active_turn_id
  ...
  defp start_controlled_session(workspace, prefix) do
    id = unique_id(prefix)
    assert {:ok, ref} = InteractiveSession.start(id: id, provider: :native, workspace: workspace)
    assert {:ok, _turn} = InteractiveSession.send_message(ref, "hold while supervision is tested")
    assert_receive {:ouroboros_test_model_started, _, _, controller}, 5_000
    assert {:ok, state} = InteractiveSession.info(ref)
    assert {:ok, runtime} = Ouroboros.Session.info(state.harness_session_id)
    %{ref: ref, coordinator: Task.whereis(id), execution: runtime.pid, controller: controller}
  end
