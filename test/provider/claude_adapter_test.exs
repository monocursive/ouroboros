defmodule Ouroboros.Provider.ClaudeAdapterTest do
  ...
  describe "with an ouro binary and a gateway" do
    ...
    # The bridge composes an MCP server and a `--permission-prompt-tool`, and nothing
    # else: `--settings` is the operator's alone. It used to carry a `PostToolUse` hook
    # that fed the model diagnostics; the plane that answered it is gone
    # (docs/proposals/core.md §4 A3).
    test "no session, bridged or not, is launched with a settings block of ours" do
      for mode <- [:prompt, :auto_edit, :auto_approve] do
        argv = argv_for(interactive_request(approval_mode: mode))
        refute "--settings" in argv, "#{mode} carried a settings block"
      end

      refute "--settings" in argv_for(request(approval_mode: :prompt, metadata: %{}))
    end
  end
  ...
  test "with no ouro binary there is no bridge to compose" do
    argv = argv_for(interactive_request(approval_mode: :prompt))
    refute "--permission-prompt-tool" in argv
