defmodule Ouroboros.Provider.Native.Hooks do
  ...
  **ignored loudly**, exactly as a shell hook that crashed is. It is not consent and it is
  ...
  Operator configuration is not on its own enough to make the file safe to read, either
  (S1): `ouroboros.toml` is a file in a workspace the session may write, so trusting it
  means trusting whatever the session writes into it next unless the kernel is holding that
  one path shut. That fence is
  `Ouroboros.Provider.Native.Sandbox`'s `protected_files`, and both backends render it —
  Seatbelt with a `literal` write deny, bubblewrap with a read-only bind — so operator
  configuration and the kernel rule now always arrive together. Component hooks are
  unaffected either way, for D8's reason below.
  ...
  **The operator named it**, and that is the whole of it:
  `config :ouroboros, :trusted_workspaces` lists the workspace, compared canonically so one
  directory has one answer however it was spelled.

  Trusting the file means trusting whatever it says next, so it has to be a file a session's
  own shell cannot rewrite (S1). The permission engine refuses a write
  ...
  `python3 -c` all reach the file without a redirect for the engine to read. The fence that
  matters is therefore `Ouroboros.Provider.Native.Sandbox`'s `protected_files` — and both
  backends this runtime has express it, Seatbelt with a `literal` write deny and bubblewrap
  with a read-only bind of the path over itself or over an empty directory. This function
  used to ask a third backend whether it could; it could not, and it is gone
  (docs/proposals/core.md §4 A2).

  `opts` takes `:trusted_workspaces`.
  """
  @spec trusted?(String.t() | nil, keyword()) :: boolean()
  def trusted?(workspace_root, opts \\ [])
  def trusted?(nil, _opts), do: false
  def trusted?(root, opts) when is_binary(root), do: configured?(root, opts)
