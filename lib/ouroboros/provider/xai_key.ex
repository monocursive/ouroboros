defmodule Ouroboros.Provider.XAIKey do
  ...
  @doc "Atomically replaces the private stored key."
  ...
  def path(opts \\ []) when is_list(opts) do
    configured = Keyword.get(opts, :path) || Application.get_env(:ouroboros, :xai_api_key_file)

    case configured do
      value when is_binary(value) and value != "" ->
        if Path.type(value) == :absolute,
          do: Path.expand(value),
          else: raise(ArgumentError, "xAI API key path must be absolute")
  ...
  defp projection(present, source),
    do: %{provider: :xai, env: @env, present: present, source: source}
  ...
  defp validate_stat(path, stat) do
    ...
    cond do
      ...
      stat.size > @max_key_bytes -> {:error, {:unsafe_credential_file, path, :too_large}}
      true -> :ok
    end
  end
  ...
  defp publish(path, key) do
    ...
    try do
      ...
      unless same_file?(before, File.lstat!(temporary, time: :posix)) do
        raise "xAI API key temporary inode changed while it was written"
      end
