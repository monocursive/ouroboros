defmodule Ouroboros.Web.AuditLive do
  ...
  alias Ouroboros.Web.{Call, Config, Layouts, Presentation}
  ...
  # `audit_reply/1` turns every plane refusal into `"Audit operation failed: \#{inspect(reason)}"`
  # (`gateway/methods.ex:611`), so the first line of this page was the raw atom
  # `:audit_disabled` whenever recording was off (review §3.5). One translation, in the one
  # module that owns it.
  defp failure(socket, error), do: assign(socket, error: Presentation.refusal(error))
