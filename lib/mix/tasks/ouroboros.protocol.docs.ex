  ...
  defp connection_section do
    ...
    [
      ...
      **#{duration(entry.timeout)}** closes it too. `hello.methods` is the feature gate and
      ...
      Every handler except the six answered by the connection itself runs in a supervised
      task under the ceiling its table entry names. A handler that outlives its ceiling is
      killed and the request is answered `-32005`.
