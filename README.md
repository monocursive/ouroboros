...
ignored. `Ouroboros.Provider.plan_mode/2` is the declaration. On the wire, `plan` is a
start option on `interactive.start` and a field on `interactive.configure`, but it takes
its own path rather than the Harness configuration one — the pinned harness normalizes a
configuration against a literal four-key list and would reject a fifth — so a native
session is told directly and applies it now, Claude can only be *started* planning, and a
mid-life change on any other transport is refused by declaration. A plan-exit answer may
carry `provider_options: {choice, follow_up}`, the one shape the gateway admits under
that key.
