...
| The §5 budgets were unmeasured, "median overhead" was undefined, and ptrace cannot meet 20% on the syscall-dense fixed workload (J0: two stops per closed-set call) | `cargo xtask perf` measures §5.2's workloads and arms; overhead is defined on work, on post-start and on end-to-end wall; the budgets apply to the jail's own overhead (`--observe off` against direct execution) and observation cost is reported per workload. Operator decision 2026-09-24, refined by the next row | §5.2; backend-evaluation.md §4 |
| The milestone measurement ([backend-evaluation.md §4](backend-evaluation.md#4-performance-52-budgets), 30 valid launches in all 42 arms at the final revision `027de7d2`): the startup budget is met (`tool` p95 added 101 to 139 ms), but the 20% fixed-workload budget is missed even by the jail's own overhead: `tool` `--observe off` against direct is +39.5% (plain) and +40.3% (scope) post-start, +29.6% and +28.9% work phase, +99.9% and +90.7% end-to-end wall. Attribution, measured at `4380241f` (bubblewrap and the distribution's confinement are unchanged since): bubblewrap alone under Ubuntu's `unpriv_bwrap` AppArmor confinement costs +19.0% of the work phase, the jail +22.2% | The startup budget stays. The fixed-workload budget becomes a measured ceiling: the jail's own overhead at most 50% median post-start on this syscall-dense worst-case workload (met), bubblewrap's containment under the distribution's confinement named as the dominant cost; work phase and end-to-end wall reported; observation cost (+389% to +446% work phase against direct on this workload) reported per workload with no budget; a representative workload joins the §5 set as later work. Operator decision 2026-09-25, superseding 2026-09-24's | §5.2; backend-evaluation.md §§4, 6; `evidence/perf-2026-09-25-ouro-ci/`, `evidence/perf-2026-09-25-attribution-ouro-ci.txt` |
| After the tracer stopped waiting for `ECHILD`, `none --observe on` left the target's adopted orphans unreaped (120 zombies in the fork-burst test) | The tracer owes, reaps and delivers every child gained after the attach, an orphan of the traced tree; a child it was attached beside is still never waited for | §§11.1, 11.4; L01 (`j5_lifetime_linux.rs::l01_a_fork_burst_is_fully_reaped`) |
| `eprintln!` panics on a failed write, so a refusal whose stderr could not be written exited 101 instead of 125 (J5-B2) | A diagnostic that cannot be written is dropped: diagnostics never change an exit code | §6.1; S04 (`j5_boundary_linux.rs`, S04.6) |
| An operator grant of host `/proc`, `/sys` or `/sys/fs/cgroup` (`--ro /proc` and the like) was accepted, and the run settled `enforced`/`enforced` while exposing the host's pids, cgroup hierarchy and the supervisor's command line (late-waves review, HIGH) | An operator grant whose resolved source is on a `proc`, `sysfs` or cgroup filesystem, or an ancestor of such a mount, refuses before exec with `policy_widening` and its key path, decided by filesystem type and mount topology, never by spelling | §9.1; S02 (`j5_pseudofs_linux.rs`, S02.7; `portable_policy.rs`) |
| A `none` target could reopen the supervisor's live trace and control streams through `/proc/<supervisor>/fd`, read its environment and ptrace it, with Yama the only lock (late-waves review, MEDIUM) | The supervisor makes itself non-dumpable once the target is about to run, in every profile; signals, the data directory's records (R05) and the cgroup stay open to a same-UID peer | §9.3; X06 (`r05_the_supervisors_own_proc_is_closed_to_a_same_uid_peer`) |
| A preparation budget that expired mid-step was reported as that step's failure (`tool`: `backend_unavailable`, `host_setup`, a false host diagnosis) or, under `none`, after `prepared` had been announced; credential staging waited on `CLOCK_MONOTONIC` (J5-B3 review) | A step whose wait the budget ended refuses as `prepare_timeout` (`retry`), checked before `prepared` is persisted and announced; staging waits on the budget's boot-clock deadline; waits that are not §6.4 budgets are named as monotonic | §§6.4, 8.2; L04 (`j5_lifetime_linux.rs::l04_the_preparation_budget_follows_the_boot_clock`) |
