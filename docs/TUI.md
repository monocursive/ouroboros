...
| `fleet.deployment.cancel` `{operation}` | Stops at a safe boundary and reports residue. It does not claim to undo anything: the program finishes or reconciles the durable step it is inside and reaps its SSH children, and a credential already delivered to another machine stays delivered. `outcome: unknown` on a ceiling breach |
| `fleet.deployment.resume` `{operation}` | Runs an interrupted operation's program again against the journal it left. `--operation ID` is the whole of resume: a step recorded `ok` is not repeated, and an `install` onto a target already in this fleet as this machine is `skipped`. The command line is the one this runtime built, or — on a runtime that restarted and has forgotten — the one the journal's `kind`, `target` and `paths` rebuild; nothing on it was ever a secret, which is what makes rebuilding it safe. Refused when a process is still holding the operation (`already_attached`), when the journal records a finished operation (`operation_finished`), and when it records no state at all (`operation_state_unknown`) — resuming an operation whose record cannot be read would be starting a second program against a machine whose state nobody knows. There is no `takeover`: with no owner recorded on an operation there is nothing to take it from |
...
ouro fleet service remove [--json]
                      ...
                      installed first would only crash-loop, and overwrites what it
                      wrote at that path before; a file there this code did not
                      write is reported with its digest and left alone. `status`
                      reports installed/loaded/running/last exit with `unknown`
                      wherever the manager did not say, and states the platform's
                      real limit: a LaunchAgent starts at login and never before
                      it, and a systemd user unit needs lingering to survive
                      logout. `disable` is the first half of stopping a supervised
                      runtime; `remove` disables and then deletes the one file this
                      code wrote. See docs/FLEET.md
