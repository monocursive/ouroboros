...
| `computer_use.status` `{node?}` | `Provider.Native.Desktop.status/0` — Computer Use readiness on the node, starting nothing: the config posture (`enabled`, `flag`, `helper_path`, `helper_present`, `denied_app_ids`) always, plus the live helper pool's cached handshake `doctor`, `phase`, and `sessions` when it is already running (`running: true`), else `running: false`. Node-routed over a bounded `:erpc` like `mcp.list`, because the helper runs where its session runs. `ouro desktop doctor` is the operator surface over it |
| `computer_use.artifact` `{sha256, node?}` | `Provider.Native.Desktop.artifact/1` — one staged screenshot by content hash, base64 with its media type and size. Searches only the `<session_dir>/desktop` directories the live pool has staged into, so an evicted or unknown sha — or a `sha256` that is not 64 hex characters, which is how a path-traversal attempt is refused — is `not_found` (`-32007`). No path ever crosses the wire, and the pool is never started to answer a fetch |
