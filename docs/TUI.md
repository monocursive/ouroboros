...
| `policy.status` `{}` | `Control.PolicyPromotion.status/0` beside `Control.PolicyEvidence.count/0` (docs/SELF.md §S2) — this node's earned-widening record: `policy` (`{name, component_sha256}` or `null`), `tools` promoted under it with the actor and the replay numbers each was promoted on, the newest twenty `demotions`, `allowable_tools` (promoted and not demoted since), the record's `durability`, the promotion `thresholds`, and `evidence`. **`evidence` is the whole of what this protocol says about the corpus**: `records`, `by_tool`, `without_document` and `unreadable` — counts and never a row. The corpus holds the exact document a policy component would have been shown for every request a human answered, command lines and paths included, and there is no verb that serves one. No `node` parameter: the record is a checkpoint on this machine and the corpus is a file on it, so this asks a machine rather than routing to one |
...
| `policy.replay` `{name, since?}` | `Wasm.PolicyEngine.replay/2` (docs/SELF.md §S2) — asks a live lane-W `policy` component every human answer in this node's corpus, dry, and counts what it said against what the human said. Per tool: `decisions`, `agreements`, `contradictions` (`allow` where the human denied), `would_resolve` (`allow` where the human approved — the prompts a promotion would remove), `stricter`, `asks`, `unreadable`, and up to twenty `contradiction_rows` carrying a `fingerprint`, a `session_id` and an `at` and **never a document**. Sealed with `report_sha256` over the canonical JSON of everything but that key and `replayed_at`, so two replays of one corpus produce one digest. `operate` rather than `read` because it stands a component up — the `computer_use.status`/`probe` split — but it decides nothing: a dry instance under its own name, no `:permission` entry, no evidence row, and the live instance untouched. `since` is an ISO 8601 instant. Node-local (plan §0 row 9: a fleet replay is not in v1) |
| `policy.promote` `{name, tool, report}` | `Wasm.PolicyEngine.promote/5` — the one way into `policy_allowable_tools` that is not an operator typing a tool name. Three gates: the report must name the bytes this node would evaluate and hash to its own `report_sha256`; the node **re-runs the replay itself**, because a report is evidence that a replay happened and not that it is still true; and the re-run must show at least 50 decisions and exactly 0 contradictions for that tool. The re-run's numbers go into the record with the report's digest beside them. The actor is the connection's principal (`Audit.Identity.actor/0`) and never a parameter — an unattributed caller is refused `-32003` with `reason: unattributed_actor` rather than recorded as one. Answers the record, in `policy.status`'s shape. `outcome: unknown` on a ceiling: the replay and the checkpoint do not stop because this socket did |
| `policy.demote` `{name, tool, reason}` | `Control.PolicyPromotion.demote/4` — narrowing, and idempotent: a tool that is not promoted, or a name this record does not hold, is `:ok` with no write. `reason` is at most 512 bytes and is **echoed, not stored**: the record's `reason` is an enumerated atom (`operator_demotion` here, `human_contradiction` for the engine's canary), because a checkpoint fsynced on every write is not where free text belongs. Answers the record |
| `policy.clear` `{}` | `Control.PolicyPromotion.clear/1` — forgets the policy name, the bytes, and every tool promoted under them. The one way to point this node's record at a different policy or at re-deployed bytes, and deliberately a separate act: no parameters, and the same named-actor requirement `policy.promote` has. Answers the record |
...
| `grok.account.read` `{}` | `Provider.GrokAuth.read/1` — non-secret first-party Grok CLI subscription readiness and device-login state. Ouroboros inspects only auth mode and an optional account label; bearer and refresh tokens remain in the CLI's private `~/.grok/auth.json`. |
| `grok.account.login.start` `{}` | `Provider.GrokAuth.login/1` — starts `grok login --device-auth` only after an operate-scope request and returns only its HTTPS verification URL, short user code, and a caller-visible login id. The first-party CLI owns browser authentication, token persistence, and refresh. |
| `grok.account.login.cancel` `{login_id}` | `Provider.GrokAuth.cancel/2` — terminates only the matching pending CLI device flow. |
...
posture**. Plan mode is not a Harness configuration key — the native transport applies it
