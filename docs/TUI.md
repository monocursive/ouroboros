...
| `computer_use.artifact` `{sha256, node?}` | `Provider.Native.Desktop.artifact/1` — one staged screenshot by content hash, base64 with its media type and size. Searches the live pool's session dirs and `<data_dir>/native/<session>/desktop/`. An unknown or non-64-hex sha is `not_found` (`-32007`). No path ever crosses the wire, and the pool is never started to answer a fetch |
...
| `computer_use.probe` `{node?}` | `Provider.Native.Desktop.probe/0` — when the node flag is on, starts the helper if needed and returns handshake `doctor` plus live `phase`/`sessions`. When the flag is off it starts nothing and returns the same posture as `computer_use.status`. Node-routed; `ouro desktop doctor --probe` is the operator surface |
