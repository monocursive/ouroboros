...
([app.rs:5929](../tui/src/ui/app.rs)) and `Ctrl-E` opens `$EDITOR`.
...
| 5 | Steering mid-turn | 1 | Esc/Ctrl-C → `interactive.interrupt` works. `follow_up` is a durable server-side queue, but the client refuses a second Enter while one call is unacknowledged and shows no queue ([app.rs:6145-6154](../tui/src/ui/app.rs)). `steer` is `{:error, :unsupported}` for every provider except `pi` — the durable-steer work on this branch is live for one provider. |
...
|---|---|---|
| Structured turn input `{prompt, attachments[≤32], reasoning_effort}` with workspace-canonicalised attachments | [methods.ex:1419-1448](../lib/ouroboros/gateway/methods.ex), [task.ex:859-895](../lib/ouroboros/interactive/task.ex) | always a bare string ([app.rs:6191-6194](../tui/src/ui/app.rs)) |
...
| X2 | `s`/`/steer` are offered on every session; `steer/3` is `{:error, :unsupported}` on every provider but `pi`. | [acp.ex:107](../lib/ouroboros/provider/session/dialect/acp.ex), [codex.ex:101](../lib/ouroboros/provider/session/dialect/codex.ex), palette [app.rs:1812](../tui/src/ui/app.rs) | Advertise `steer` from `interactive.info.transport.capabilities`; the composer's Steer verb is shown only where it is true; where it is false, Enter-while-busy becomes a visible queued follow-up (Track B3). |
| X3 | TUI.md documents `Ctrl-E` for event details; the code binds `Ctrl-O` and `Ctrl-E` opens `$EDITOR`. | [TUI.md §3.4](TUI.md), [app.rs:5929](../tui/src/ui/app.rs) | Fix the doc; `?` already is the authority. |
